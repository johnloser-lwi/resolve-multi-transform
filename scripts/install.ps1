# Install MultiTransform.ofx.bundle into the system OFX plugin directory.
#
# Self-elevating: if run from a normal shell it relaunches itself via UAC.
# You can also just double-click Install-MultiTransform.cmd next to this file.
#
# Resolve scans for OFX plugins only at launch, so it must be closed during the
# copy (it holds a file lock on any plugin it has loaded) and restarted after.
...
function Test-Writable([string]$dir) {
    try {
        ...
        $probe = Join-Path $dir ".mtx-write-probe-$PID"
...
try {
    ...
    if (-not (Test-Path $bundle)) {
        throw "Bundle not found at:`n  $bundle`nRun scripts\build.ps1 first, or re-run this with -Build."
    }

    $payload = Join-Path $bundle 'Contents\Win64\MultiTransform.ofx'
    if (-not (Test-Path $payload)) {
        throw "Bundle is malformed - missing Contents\Win64\MultiTransform.ofx"
    }
    ...
    # --- Verify --------------------------------------------------------------
    $installed = Join-Path $target 'Contents\Win64\MultiTransform.ofx'
    ...
    Write-Host "  2. Apply 'Multi Transform' (group: MultiTransform) from the OpenFX library."
    Write-Host "  3. Under the Viewer, set the on-screen-control dropdown to 'Open FX Overlay'."
    Write-Host ""
    Write-Host "Probe log: $env:LOCALAPPDATA\MultiTransform\probe.log"
    Write-Host "  (written by the plugin on load - send this back to confirm host capabilities)"
